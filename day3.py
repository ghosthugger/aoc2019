...
#    w2 = ["U62","R66","U55","R34","D71","R55","D58","R83"]
    ...
    zip1 = zip(path1, range(len(path1)))
    zip2 = zip(path2, range(len(path2)))

    set1 = {coord: pos for (coord, pos) in zip1 }
    set2 = {coord: pos for (coord, pos) in zip2 }

    print(set2)

    common_coords = set1.keys() & set2.keys()
    print(common_coords)

    res = []
    for el in common_coords:
        res.append((el, set1[el]+set2[el]))
